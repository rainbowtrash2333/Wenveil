...
function App() {
  ...
  const irreversibleCount = steps.mask ? results.filter((result) => !result.reversible).length : 0;
  ...
  async function handleProcess() {
    ...
    try {
      const response = await bridgeState.bridge.process({ files, steps, password: steps.mask && password ? password : undefined, outputDir, entities: [...entities], aiEnhanced, ocrMode, device, keepIntermediate }, (event) => {
...
type ProcessProps = {
  files: SelectedFile[]; steps: StepState; toggleStep: (step: StepKey) => void; addFiles: (list: FileList | File[]) => void; onInputChange: (event: ChangeEvent<HTMLInputElement>) => void; onDrop: (event: DragEvent<HTMLDivElement>) => void; fileInput: React.RefObject<HTMLInputElement | null>; folderInput: React.RefObject<HTMLInputElement | null>; clearFiles: () => void; removeFile: (fileId: string) => void; password: string; setPassword: (value: string) => void; confirmation: string; setConfirmation: (value: string) => void; showPassword: boolean; setShowPassword: (value: boolean) => void; outputDir: string; setOutputDir: (value: string) => void; chooseOutputDirectory: (setter: (value: string) => void) => void; ocrMode: "auto" | "fast" | "enhanced"; setOcrMode: (value: "auto" | "fast" | "enhanced") => void; device: "auto" | "cpu" | "gpu"; setDevice: (value: "auto" | "cpu" | "gpu") => void; entities: Set<string>; setEntities: (value: Set<string>) => void; aiEnhanced: boolean; setAiEnhanced: (value: boolean) => void; runState: RunState; message: string; progress: Record<string, FileProgress>; results: ProcessResult["files"]; overallProgress: number; completedCount: number; attentionCount: number; irreversibleCount: number; handleProcess: () => void; cancelProcess: () => void; openOutputFolder: () => void; setPage: (page: Page) => void; openOutput: boolean; setOpenOutput: (value: boolean) => void;
};

function ProcessPage(props: ProcessProps) {
  const hasMask = props.steps.mask;
  return <>
    <section className="page-heading"><div><span className="section-kicker">DOCUMENT PROCESSING</span><h1>文档处理</h1><p>把原始文档变成可阅读、可控的工作副本；设置密码后还可恢复。</p></div><div className="privacy-badge"><span className="shield">✦</span><div><strong>离线优先</strong><small>原文不离开本机</small></div></div></section>
    <section className="workspace-grid">
      ...
      <aside className="controls-column">
        <div className="control-card steps-card"><div className="panel-heading"><div><span className="section-kicker">AUTOMATION</span><h2>处理步骤</h2></div><span className="auto-pill">自动判断</span></div><p className="card-copy">根据文件类型预选步骤，你仍可以随时调整。</p><div className="step-list">{(Object.keys(STEP_LABELS) as StepKey[]).map((step, index) => <div className={`step-row ${props.steps[step] ? "enabled" : ""}`} key={step}><button className="check-toggle" aria-pressed={props.steps[step]} onClick={() => props.toggleStep(step)}><span>{props.steps[step] && <Icon name="check" />}</span></button><div><strong>{STEP_LABELS[step]}</strong><small>{step === "ocr" ? "从文档或图片中识别文字" : step === "organize" ? "整理空格、断行和 Markdown 结构" : step === "mask" ? "保护敏感信息，可选生成加密映射" : "检查可能残留的敏感信息"}</small></div><b>{String(index + 1).padStart(2, "0")}</b></div>)}</div>{props.steps.ocr && <details className="advanced-settings" open><summary><span>文字识别设置</span><span className="summary-arrow">⌄</span></summary><div className="inline-fields"><Field label="识别方式"><select value={props.ocrMode} onChange={(event) => props.setOcrMode(event.target.value as ProcessProps["ocrMode"])}><option value="auto">自动</option><option value="fast">快速识别</option><option value="enhanced">增强识别</option></select></Field><Field label="处理设备"><select value={props.device} onChange={(event) => props.setDevice(event.target.value as ProcessProps["device"])}><option value="auto">自动</option><option value="cpu">CPU</option><option value="gpu">GPU</option></select></Field></div></details>}</div>
        {hasMask && <div className="control-card mask-card"><div className="panel-heading"><div><span className="section-kicker">DOCUMENT MASKING</span><h2>文档脱敏</h2></div><span className="lock-mark">⌁</span></div><div className="entity-grid">{ENTITY_OPTIONS.map(([key, label]) => <button className={`entity-chip ${props.entities.has(key) ? "selected" : ""} ${key === "CUSTOM" ? "custom" : ""}`} key={key} onClick={() => { const next = new Set(props.entities); next.has(key) ? next.delete(key) : next.add(key); props.setEntities(next); }}><span>{props.entities.has(key) ? "✓" : ""}</span>{label}</button>)}</div><label className="switch-row"><span><strong>使用 AI 增强敏感信息识别</strong><small>模型可用时增强识别，不影响基础能力</small></span><input type="checkbox" checked={props.aiEnhanced} onChange={(event) => props.setAiEnhanced(event.target.checked)} /><i /></label><div className="password-divider" /><Field label="脱敏恢复密码" hint="可选；留空将生成不可恢复的脱敏文件，不生成映射文件"><div className="password-input"><input type={props.showPassword ? "text" : "password"} value={props.password} onChange={(event) => props.setPassword(event.target.value)} placeholder="留空则不可恢复" /><button type="button" aria-label={props.showPassword ? "隐藏密码" : "显示密码"} onClick={() => props.setShowPassword(!props.showPassword)}><Icon name={props.showPassword ? "eye-off" : "eye"} /></button></div></Field><Field label="确认密码"><input type={props.showPassword ? "text" : "password"} value={props.confirmation} onChange={(event) => props.setConfirmation(event.target.value)} placeholder="如设置密码请再次输入" /></Field></div>}
