...
> 版本：V0.3（2026-09-20）｜状态：生效
> 本文档与 `AGENTS.md`、`DEVELOPMENT-GUIDELINES.md` 配套使用；是 Git 相关最高约束。

## 1. 分支职责

- `dev`：本地开发分支，承载实现与验证；**AI 默认只在该分支工作**。
- `main`：发布/稳定分支，默认由项目维护者手动维护和推送。
- AI **不执行**向远端推送、合并到 `sync`/`main` 或重写历史的操作。唯一例外是项目维护者
  明确授权的一次性操作，授权事实记录在本条：

  - 2026-09-20：维护者授权 AI 将 `dev`（含 preserve-names 特性与文档同步提交）合并到
    `main` 并推送到 `origin/main`。此后无新的明确授权前，上述限制继续生效。
