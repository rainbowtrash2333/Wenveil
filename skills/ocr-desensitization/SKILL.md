...
6. 对设置了密码、需要交付可恢复结果的流程执行恢复验证：
   ...
   未设置密码的不可逆结果没有 mapping，跳过恢复验证，并在报告中明确其不可恢复性。
...
- 统一使用稳定、可读、无碰撞的占位符；同一实体需要保持一致映射。可恢复模式的映射文件必须加密保存，
  无密码模式不得落盘 mapping，任何 mapping 都不能提交到公开版本库。
- 机构全称、简称和子公司/分公司要保留关系语义但不泄露原值：使用 `⟦机构1⟧`、`⟦机构1-别名1⟧`、`⟦机构1-子公司1⟧` 这类短占位符；可恢复模式将真实值和 `ALIAS_OF`/`SUBSIDIARY_OF`/`BRANCH_OF` 关系放入加密 mapping，无密码模式不保存这些值。
...
- 密码缺失：继续生成不可恢复的 masked 文件，不生成 mapping，并在报告/结果中标记不可恢复；需要恢复时重新处理原文并设置
  `DESENSE_PASSWORD` 或 `--password`。不要把密码写入源码、skill、日志或最终回复。
...
- 关键 Markdown 结构保持可读；
- 可恢复结果的恢复验证通过，规范化原文与恢复结果一致；不可逆结果明确没有恢复能力；
